...
import { getAppointments, updateAppointment, deleteAppointment, getDoctorSlots } from "../services/api";
...
const AppointmentManagement = () => {
  ...
  useEffect(() => {
    const socket = io("https://appointment-booking-system-backend.onrender.com");
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      const doctorId = typeof editingAppointment.doctorId === 'object' 
  ...
  const handleUpdateAppointment = async (e) => {
    ...
    try {
      ...
      const localDateStr = `${format(selectedDate, "yyyy-MM-dd")}T${selectedSlot}:00`;
              ...
              {editingAppointment && editingAppointment._id === appointment._id ? (
                      ...
                      {availableSlots.length > 0 ? (
                        availableSlots.map((slot) => (
                          <button
                            ...
                            className={`update-time-slot ${selectedSlot === slot ? 'selected' : ''}`}
                          >
                            {slot}
                          </button>
                        ))
                      ) : (
                        <p className="update-no-slots-message">No available slots for this date</p>
                      )}
                    </div>
                  </div>
                ...
                <>
                  ...
                  <p className="appointment-detail">
                    <strong>Date:</strong> {format(parseISO(appointment.date), "MMMM d, yyyy h:mm a")}
                  </p>
                  <p className="appointment-detail">
